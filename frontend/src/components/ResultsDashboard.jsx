    ...
    return (
        <div className="flex w-full flex-col items-center">
            <div className="mb-7 text-center">
                <h2 className="mb-3 text-3xl font-semibold text-[var(--text-primary)]">Validation Complete</h2>
                        ...
                        <div className="flex gap-2">
                            {results.valid_file && (
                                <button
                                    ...
                                    className="flex items-center gap-2 rounded-lg border border-[var(--border-soft)] bg-[var(--panel)] px-4 py-2.5 text-sm font-semibold text-[var(--text-primary)] transition-colors hover:bg-[var(--panel-muted)]"
                                >
            ...
            <motion.div
                ...
                className="w-full max-w-3xl rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] p-6 shadow-sm"
                style={{ fontFamily: 'Inter, system-ui, sans-serif' }}
            >
                <div className="mb-6 flex items-center justify-between">
                    <h3 className="text-lg font-bold text-[var(--text-primary)]">Validation Report Preview</h3>
                ...
                <div className="mb-6 grid grid-cols-2 gap-4">
                    <div className="rounded-xl border border-[var(--border-soft)] bg-[var(--panel-muted)] p-4">
                        <p className="text-sm text-[var(--text-secondary)] mb-2">Total Rows</p>
                        <p className="text-2xl font-bold text-[var(--text-primary)]">{results.total_rows.toLocaleString()}</p>
                    </div>
                    <div className="rounded-xl border border-[var(--border-soft)] bg-[var(--panel-muted)] p-4">
                        <p className="text-sm text-[var(--text-secondary)] mb-2">Invalid Rows</p>
                        <p className="text-2xl font-bold text-red-600">{results.invalid_rows.toLocaleString()}</p>
                    </div>
                </div>

                <div className="mb-8">
                    <p className="text-sm text-[var(--text-secondary)] mb-4 font-medium">Rule Failures by Column</p>
                    {columnFailures.length === 0 ? (
                        <p className="text-sm text-[var(--text-secondary)] italic">No failures found.</p>
                    ) : (
                        <div className="space-y-4">
                            {columnFailures.slice(0, 5).map((item) => {
                                ...
                                return (
                                    <div key={item.label}>
                                        <div className="flex justify-between text-sm text-[var(--text-primary)] mb-1.5">
                                            <span>{item.label}</span>
                                            <span className="font-medium text-[var(--text-secondary)]">{displayPercent}%</span>
                                        </div>
                ...
                {results.rules && results.rules.length > 0 && (
                    <div className="mb-8">
                        <div className="flex justify-between items-center mb-4">
                            <p className="text-sm text-[var(--text-secondary)] font-medium">Rules Applied</p>
                            <button onClick={() => setShowJsonRules(!showJsonRules)} className="text-xs text-brand-blue font-semibold hover:underline bg-[var(--panel-muted)] px-3 py-1 rounded-lg">
                                {showJsonRules ? 'View as Cards' : 'View JSON'}
                            </button>
                        </div>
                        {showJsonRules ? (
                            <pre className="p-4 bg-slate-900 text-[var(--text-muted)] rounded-xl overflow-x-auto text-xs font-mono">
                                {JSON.stringify(results.rules, null, 2)}
                            </pre>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {results.rules.map((rule, idx) => (
                                    <div key={idx} className="p-3 rounded-xl bg-[var(--panel-muted)] border border-[var(--border-soft)]">
                                        <p className="text-sm font-semibold text-[var(--text-primary)]">{rule.rule_type} <span className="text-[var(--text-secondary)] font-normal">on</span> {rule.column}</p>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div>
                    <p className="text-sm text-[var(--text-secondary)] mb-4 font-medium">Data Quality Mix</p>
                    <div className="flex items-center gap-6">
                        ...
                        <div className="space-y-2 text-sm">
                            <p className="text-[var(--text-primary)] flex items-center">
                            ...
                            <p className="text-[var(--text-primary)] flex items-center">
                                <span className="inline-block w-2.5 h-2.5 rounded-full bg-[var(--panel-muted)] mr-3" />
            ...
            <div className="mt-8 flex flex-wrap gap-3">
                {onEditRules && (
                    <button
                        onClick={onEditRules}
                        className="flex items-center gap-2 rounded-lg border border-[var(--border-soft)] bg-[var(--panel-muted)] px-4 py-2 text-sm font-semibold text-[var(--text-primary)] transition-colors hover:bg-[var(--panel-muted)] hover:text-[var(--text-primary)]"
                    >
                ...
                <button
                    onClick={onReset}
                    className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--panel-muted)] hover:text-[var(--text-primary)]"
                >
