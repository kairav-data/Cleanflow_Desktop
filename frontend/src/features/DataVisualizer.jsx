            ...
            >
                ...
                <div className="flex items-start justify-between gap-4 px-7 py-5 shrink-0 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => downloadChartExcel(chart)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200/70 text-sm font-semibold hover:bg-emerald-100 transition-all"><FileSpreadsheet size={14} /> Excel</button>
    ...
    return (
        ...
        >
            ...
            <div className="p-5 pb-3 flex items-start justify-between gap-3">
                ...
                <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 opacity-0 group-hover:opacity-100 transition-all duration-150 bg-slate-50 border border-slate-200"><Maximize2 size={13} className="text-slate-500" /></div>
            </div>
            <div className="px-3 pb-4 flex-1 pointer-events-none">{renderChartJSX(chart, index, 170, '')}</div>
            <div className="px-5 pb-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150"><Maximize2 size={10} className="text-slate-400" /><p className="text-xs text-slate-600 font-medium">Click to expand</p></div>
        </motion.div>
    );
}
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl overflow-hidden mb-5 shadow-sm">
            <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-5 py-4 hover:bg-slate-50/80 transition-colors">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-sm shadow-violet-200"><Bot size={15} className="text-white" /></div>
                ...
                <div className="flex items-center gap-2">{aiUsed && <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-violet-50 text-violet-600 border border-violet-200/60">AI</span>}{open ? <ChevronUp size={15} className="text-slate-400" /> : <ChevronDown size={15} className="text-slate-400" />}</div>
            </button>
            <AnimatePresence>
                {open && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-slate-100 overflow-hidden">
                        <div className="p-4 flex gap-3">
                            <textarea value={newPrompt} onChange={e => setNewPrompt(e.target.value)} placeholder='e.g. "Show revenue trends over time..."' rows={2} className="flex-1 text-sm border border-slate-200 rounded-xl px-3.5 py-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-violet-400 placeholder-slate-300 bg-slate-50/50" />
                            <button onClick={() => { onRegenerate(newPrompt); setOpen(false); }} disabled={loading} className="self-end flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-sm font-semibold shadow-md hover:from-violet-500 disabled:opacity-50 transition-all whitespace-nowrap">{loading ? <RefreshCw size={14} className="animate-spin" /> : <Send size={14} />}{loading ? 'Thinking…' : 'Regenerate'}</button>
                        </div>
                        <div className="px-4 pb-4 flex flex-wrap gap-2">{suggestions.map(s => <button key={s} onClick={() => setNewPrompt(s)} className="px-3 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600 hover:bg-violet-100 hover:text-violet-700 transition-colors border border-slate-200/60">{s}</button>)}</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
                        ...
                        {columns?.map((col, i) => {
                            ...
                            return (
                                <tr key={i} className="border-t hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3.5 text-right"><span className={`font-bold ${col.nullPct > 20 ? 'text-red-500' : col.nullPct > 5 ? 'text-amber-500' : 'text-emerald-500'}`}>{col.nullPct}%</span></td>
...
function DashboardWorkspace({ analysis, dashboardSummary, filename, activeView, regenerating, onExpandChart, smartKpis, sessionId }) {
    return (
        <div className="space-y-6">
            <div className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-6 md:p-7 shadow-xl">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(129,140,248,0.28),transparent_34%)]" />
                <div className="relative flex flex-col xl:flex-row xl:items-start xl:justify-between gap-6">
                    <div className="max-w-3xl">
                        <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.22em] text-indigo-100"><Sparkles size={12} className="text-indigo-200" /> {dashboardSummary?.headline || 'Dataset intelligence'}</div>
                        <h2 className="mt-4 text-2xl md:text-3xl font-black tracking-tight text-white">{filename.replace(/\.[^.]+$/, '') || 'AI Visualizer dashboard'}</h2>
                        <p className="mt-3 max-w-2xl text-sm md:text-[15px] leading-7 text-slate-300">{dashboardSummary?.summary || 'Dynamic dashboard generated from your uploaded dataset.'}</p>
                        <div className="mt-5 flex flex-wrap gap-2.5">
                            <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3.5 py-1.5 text-xs font-semibold text-white"><LayoutDashboard size={12} /> {analysis.charts?.length || 0} charts</span>
                            <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3.5 py-1.5 text-xs font-semibold text-white"><Database size={12} /> {analysis.totalRows?.toLocaleString()} rows</span>
                            <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3.5 py-1.5 text-xs font-semibold text-white"><TableProperties size={12} /> {analysis.totalColumns} columns</span>
                        </div>
                    </div>
            ...
            {smartKpis.length > 0 && (
                <div className="grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-4">
                    {smartKpis.map((kpi, i) => {
                        ...
                        return (
                            <motion.div key={i} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className="rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
                                <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-wider text-slate-400">{kpi.label}</p><p className="mt-3 text-2xl font-black tracking-tight text-slate-900 truncate">{kpi.value}</p></div><div className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${kpi.color}15` }}><Icon size={18} style={{ color: kpi.color }} /></div></div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm mt-6">
                <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-wider text-slate-400">Executive Insights</p><h3 className="mt-1 text-base font-black text-slate-900">AI-generated dashboard briefing</h3></div><span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{dashboardSummary?.insightCards?.length || 0} live insights</span></div>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">{(dashboardSummary?.insightCards || []).map((card, index) => <InsightCard key={index} card={card} index={index} />)}</div>
            </div>
            {activeView === 'charts' ? (
                regenerating ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center"><div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center mb-5 shadow-xl"><Sparkles size={26} className="text-white animate-pulse" /></div><p className="text-slate-700 font-black text-lg mb-1.5">Rethinking your dashboard...</p></div>
                ) : analysis.charts?.length > 0 ? (
                    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-5"><div><p className="text-[11px] font-black uppercase tracking-wider text-slate-400">Live Dashboard</p><h3 className="mt-1 text-base font-black text-slate-900">Interactive chart board</h3></div></div>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">{analysis.charts.map((chart, i) => <ChartCard key={i} chart={chart} index={i} onExpand={() => onExpandChart(chart, i)} />)}</div>
                    </div>
                ) : null
            ) : activeView === 'dataset' ? (
                ...
                <div className="space-y-6"><SchemaMixPanel profile={dashboardSummary?.profile} /><ColumnTablePanel columns={analysis.columnSummary} /></div>
            )}
        </div>
    );
}
...
export default function DataVisualizer({
    initialSessionId = null, initialFilename = '', initialRowCount = 0, initialPrompt = '', onClose = null,
}) {
    ...
    const analyzeExistingSession = useCallback(async (sid, options = {}) => {
        if (!sid) return;
        const { prompt: nextPrompt = '', filename: nextFilename = '', rowCount: nextRowCount = 0 } = options;
        setError(''); setAnalysis(null); setExpandedChart(null); setActiveView('charts'); setStep(2);
        setSessionId(sid); setFilename(nextFilename || 'Pipeline Output'); setRowCount(nextRowCount || 0);
    ...
    return (
        <div className="flex flex-col h-full w-full bg-slate-50/80">
            ...
            <div className="shrink-0 bg-white border-b border-slate-100 px-6 md:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-md"><BarChart3 size={19} className="text-white" /></div>
                    <div><h1 className="text-base font-black text-slate-900 flex items-center gap-2">AI Visualizer{HF_API_KEY && <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-violet-50 text-violet-600 border border-violet-200/60">Qwen</span>}</h1><p className="text-xs text-slate-600 font-medium mt-0.5">Connect data → instant AI dashboard</p></div>
                </div>
                <div className="flex items-center gap-2 flex-wrap justify-end">
                    {[{ n: 1, label: 'Upload' }, { n: 2, label: 'Visualize' }].map((s, i) => (
                        <div key={s.n} className="flex items-center gap-2">
                            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${step === s.n ? 'bg-violet-600 text-white shadow-sm shadow-violet-300' : step > s.n ? 'bg-emerald-100 text-emerald-700 border border-emerald-200/60' : 'bg-slate-100 text-slate-400'}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                                <div className="space-y-4">
                                    <div className="flex bg-slate-100 rounded-xl p-1 mb-2 w-fit">
                                        {['file', 'database'].map((m) => (
                                            <button key={m} onClick={() => setMode(m)} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${mode === m ? 'bg-white text-violet-600 shadow-sm' : 'text-slate-800 hover:text-slate-900'}`}>{m === 'file' ? 'File Upload' : 'Local Database'}</button>
                                        ))}
                                    </div>
                                    {mode === 'file' ? (
                                        <><div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onClick={() => fileInputRef.current?.click()} className={`relative rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${dragging ? 'border-violet-400 bg-violet-50/60 scale-[1.01]' : file ? 'border-emerald-400 bg-emerald-50/30' : 'border-slate-200 hover:border-violet-300 hover:bg-violet-50/20 bg-white'}`}>
                                            <input ref={fileInputRef} type="file" accept=".csv,.tsv,.txt,.xlsx,.xls" className="hidden" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                                            <div className="py-14 px-8 flex flex-col items-center text-center">{file ? (<><div className="w-14 h-14 rounded-2xl bg-emerald-100 flex items-center justify-center mb-4"><FileText size={26} className="text-emerald-600" /></div><p className="text-base font-black text-slate-800 mb-1">{file.name}</p><p className="text-sm text-slate-600 font-medium">{(file.size / 1024).toFixed(1)} KB · Click to change</p></>) : (<><div className="w-16 h-16 rounded-2xl bg-violet-50 border border-violet-100 flex items-center justify-center mb-5"><Upload size={28} className="text-violet-400" /></div><p className="text-lg font-black text-slate-800 mb-1.5">Drop your dataset here</p><p className="text-sm text-slate-600 font-medium">CSV, Excel, TSV, TXT supported</p></>)}</div>
                                        ...
                                        </div><div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm"><p className="text-xs font-bold uppercase text-slate-400 tracking-widest mb-3">Column Separator</p><div className="flex items-center flex-wrap gap-2">{SEPARATORS.map(s => <button key={s.value} onClick={() => { setDelimiter(s.value); setCustomDelim(''); }} className={`px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${delimiter === s.value && !customDelim ? 'bg-violet-600 text-white border-violet-600 shadow-sm shadow-violet-200' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-violet-300 hover:text-violet-600'}`}>{s.label}</button>)}<input type="text" placeholder="Custom…" value={customDelim} onChange={(e) => setCustomDelim(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 text-sm font-mono w-24 focus:outline-none focus:ring-2 focus:ring-violet-400 bg-slate-50" /></div></div></>
                                    ) : (
                                            ...
                                            {connections.length === 0 ? (
                                                ...
                                                <>
                                                    <div>
                                                        ...
                                                        <select value={connectionId} onChange={(e) => setConnectionId(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm font-medium focus:ring-2 focus:ring-violet-400 outline-none">
                                                    ...
                                                    <div>
                                                        ...
                                                        <textarea rows={6} value={query} onChange={(e) => setQuery(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-violet-400 outline-none resize-none" />
                                                    </div>
                                                </>
                                            )}
                                        </div>
                                ...
                                <div className="flex flex-col h-full">
                                    {HF_API_KEY ? (<div className="bg-gradient-to-br from-violet-50/80 to-indigo-50/60 rounded-2xl border border-violet-100 p-5 shadow-sm flex-grow mb-4"><div className="flex items-center gap-2 mb-3"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-sm"><Wand2 size={13} className="text-white" /></div><p className="text-sm font-bold text-slate-800">Describe your dashboard <span className="text-violet-500 font-medium ml-1">(optional)</span></p></div><textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder='e.g. "Show me revenue by region..."' rows={3} className="w-full text-sm border border-violet-200/70 bg-white rounded-xl px-3.5 py-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-violet-400 placeholder-slate-300 shadow-sm" /><div className="mt-2.5 flex flex-wrap gap-2">{['Top categories by count', 'Numeric correlations', 'Revenue over time'].map(s => <button key={s} onClick={() => setPrompt(s)} className="px-3 py-1 rounded-full text-xs font-medium bg-white text-violet-600 border border-violet-200/70 hover:bg-violet-100 transition-colors shadow-sm">{s}</button>)}</div></div>) : (<div className="bg-slate-50 rounded-2xl border border-slate-200 p-6 flex-grow mb-4 flex flex-col items-center justify-center text-center"><div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center mb-3"><BarChart3 size={22} className="text-slate-400" /></div><h3 className="text-slate-700 font-bold mb-1.5">Standard Dashboard</h3><p className="text-sm text-slate-400 max-w-sm">Automatic dashboard built from your data heuristics.</p></div>)}
