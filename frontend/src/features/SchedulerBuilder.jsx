...
export default function SchedulerBuilder() {
    ...
    const inputClass = (hasError) =>
        `w-full rounded-xl border px-3 py-3 text-sm outline-none transition-all focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200 ${
            hasError ? 'border-red-300 bg-red-50' : 'border-[var(--border-soft)] bg-[var(--panel)] hover:border-[var(--border-strong)]'
        }`;

    return (
        <div className="flex h-full w-full flex-col bg-[var(--panel-muted)]">
            <div className="shrink-0 border-b border-[var(--border-soft)] bg-[var(--panel)] px-8 py-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-semibold leading-tight text-[var(--text-primary)]">Pipeline Scheduler</h1>
                            <p className="text-xs font-medium text-[var(--text-muted)]">Run saved pipelines automatically on a real backend schedule</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border-soft)] bg-[var(--panel-muted)] px-3 py-1.5 text-xs font-bold text-[var(--text-secondary)]">
                            {stats.paused} Paused
                        </span>
                        <button
                            onClick={() => loadData({ silent: true })}
                            className="inline-flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] px-4 py-2.5 text-sm font-bold text-[var(--text-primary)] shadow-sm transition-all hover:bg-[var(--panel-muted)]"
                        >
                    ...
                    <div className="overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--panel)] shadow-sm">
                        ...
                        <div className="p-6">
                            <div className="mb-6 flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-base font-semibold text-[var(--text-primary)]">{isEditing ? 'Edit Schedule' : 'New Schedule'}</h2>
                            ...
                            {!loading && pipelines.length === 0 ? (
                                <div className="rounded-2xl border border-dashed border-[var(--border-soft)] bg-[var(--panel-muted)] px-5 py-8 text-center">
                                    <p className="font-bold text-[var(--text-primary)]">No saved pipelines yet</p>
                                    <p className="mt-1 text-sm text-[var(--text-secondary)]">Save a pipeline from the pipeline builder first, then schedule it here.</p>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Frequency</label>
                                        <div className="mt-2 grid grid-cols-2 gap-2">
                                            {FREQUENCIES.map((frequency) => (
                                                <button
                                                    ...
                                                    className={`rounded-xl border p-3 text-left transition-all ${
                                                        draft.frequency === frequency.value
                                                            ? 'shadow-sm'
                                                            : 'border-[var(--border-soft)] bg-[var(--panel-muted)] hover:border-[var(--border-strong)]'
                                                    }`}
                                                ...
                                                >
                                                    <span className="block text-xs font-semibold">{frequency.label}</span>
                                    ...
                                    <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                                        <button
                                            ...
                                            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 py-3.5 text-sm font-semibold text-white shadow-lg shadow-indigo-200 transition-all hover:from-indigo-500 hover:to-violet-500 disabled:cursor-not-allowed disabled:opacity-60"
                                        >
                                        ...
                                        {isEditing ? (
                                            <button
                                                ...
                                                disabled={saving}
                                                className="w-full rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] px-4 py-3.5 text-sm font-bold text-[var(--text-secondary)] transition-all hover:bg-[var(--panel-muted)] disabled:cursor-not-allowed disabled:opacity-60"
                                            >
                                                Cancel Edit
                                            </button>
                                        ) : null}
                    ...
                    <div>
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="text-base font-semibold text-[var(--text-primary)]">
                                Scheduled Pipelines <span className="ml-2 text-sm font-semibold text-[var(--text-muted)]">({stats.total})</span>
                            </h2>
                        </div>

                        {loading ? (
                            <div className="rounded-2xl border border-[var(--border-soft)] bg-[var(--panel)] px-6 py-12 text-center shadow-sm">
                                <RefreshCw size={18} className="mx-auto animate-spin text-[var(--text-muted)]" />
                                <p className="mt-3 text-sm font-medium text-[var(--text-secondary)]">Loading schedules...</p>
                            </div>
                        ) : schedules.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[var(--border-soft)] bg-[var(--panel)] py-24">
                                <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-[var(--border-soft)] bg-[var(--panel-muted)]">
                                    <Calendar size={28} className="text-[var(--text-muted)]" />
                                </div>
                                <p className="font-bold text-[var(--text-secondary)]">No schedules yet</p>
                                <p className="mt-1 text-xs text-[var(--text-muted)]">Create your first recurring pipeline run from the panel on the left.</p>
                            </div>
                        ) : (
                                ...
                                <AnimatePresence>
                                    {schedules.map((schedule, index) => (
                                        <Motion.div
                                            ...
                                            className="overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--panel)] shadow-sm"
                                        >
                                            <div className={`h-0.5 w-full ${schedule.is_active ? 'bg-emerald-400' : 'bg-[var(--panel-muted)]'}`} />
                                            <div className="p-5">
                                                <div className="flex items-start justify-between gap-4">
                                                    <div className="flex min-w-0 items-start gap-4">
                                                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${schedule.is_active ? 'bg-emerald-50' : 'bg-[var(--panel-muted)]'}`}>
                                                            <GitMerge size={18} className={schedule.is_active ? 'text-emerald-600' : 'text-[var(--text-muted)]'} />
                                                        </div>
                                                        <div className="min-w-0">
                                                            <div className="mb-1 flex flex-wrap items-center gap-2">
                                                                <p className="truncate text-sm font-semibold text-[var(--text-primary)]">{schedule.schedule_name}</p>
                                                                <FrequencyBadge frequency={schedule.frequency} />
                                                            </div>
                                                            <p className="truncate text-xs text-[var(--text-secondary)]">{schedule.pipeline_name || 'Saved pipeline'}</p>
                                                            <p className="mt-1.5 flex items-center gap-1 text-xs text-[var(--text-muted)]">
                                                                <Clock3 size={10} /> {buildScheduleSummary(schedule)}
                                                            </p>
                                                            <p className="mt-2 text-xs text-[var(--text-secondary)]">Next run: {formatDateTimeInIST(schedule.next_run_at, 'Not scheduled yet')}</p>
                                                            <p className="mt-1 text-xs text-[var(--text-muted)]">Last run: {formatDateTimeInIST(schedule.last_run_at, 'Not scheduled yet')}</p>
                                                            {schedule.notes ? (
                                                                <p className="mt-2 truncate text-xs italic text-[var(--text-muted)]">"{schedule.notes}"</p>
                                                            ) : null}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex shrink-0 items-center gap-2">
                                                        <button
                                                            ...
                                                            className="rounded-xl border border-[var(--border-soft)] bg-[var(--panel)] p-2 text-[var(--text-muted)] transition-all hover:border-indigo-100 hover:bg-indigo-50 hover:text-indigo-600 disabled:cursor-not-allowed disabled:opacity-50"
                                                            title="Edit schedule"
                                                        >
                                                            <Pencil size={15} />
                                                        </button>
                                                        <button
                                                            ...
                                                            className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold transition-all ${
                                                                schedule.is_active
                                                                    ...
                                                                    : 'border-[var(--border-soft)] bg-[var(--panel-muted)] text-[var(--text-secondary)] hover:border-emerald-100 hover:bg-emerald-50 hover:text-emerald-700'
                                                            }`}
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={busyScheduleId === schedule.id}
                                                            className="rounded-xl border border-transparent p-2 text-[var(--text-muted)] transition-all hover:border-red-100 hover:bg-red-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-50"
