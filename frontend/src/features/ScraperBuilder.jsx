...
export default function ScraperBuilder({ onComplete }) {
    ...
    return (
        <div className="w-full h-full flex flex-col" style={{background: 'linear-gradient(135deg, #f0f6ff 0%, #ffffff 60%, #e8f0fe 100%)'}}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-blue-100/80 bg-white/90 backdrop-blur-sm shrink-0 shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-md" style={{background: 'linear-gradient(135deg, #2563eb, #1d4ed8)'}}>
                        <Globe size={18} className="text-white" />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-slate-900 tracking-tight">AI Web Scraper</h2>
                        <p className="text-[11px] text-blue-500 font-medium">Localized scraping, configurable screenshots, and export-ready data</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-xl">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-blue-500 uppercase tracking-wider leading-none">Credits</span>
                            <span className="text-xs font-black text-blue-900 leading-none mt-0.5">
                                {loadingCredits ? '...' : formatCreditValue(credits?.remaining)}
                            </span>
                        </div>
                        {typeof credits?.extra_credits === 'number' && credits.extra_credits > 0 && (
                            <span className="rounded-lg bg-blue-100 px-2 py-1 text-[10px] font-bold text-blue-700 border border-blue-300">
                                +{formatCreditValue(credits.extra_credits)} extra
                            </span>
                        )}
                        <button onClick={fetchCredits} className="ml-1 p-1 hover:bg-blue-100 rounded-md text-blue-400 hover:text-blue-600 transition-colors">
                            <RefreshCw size={12} className={loadingCredits ? 'animate-spin' : ''} />
                        </button>
                    </div>

                    <div className="w-px h-6 bg-blue-100 mx-1"></div>
                            ...
                            return (
                                <div key={currentStep} className="flex items-center">
                                    ...
                                    {currentStep < STEPS.length && (
                                        <div className={`w-4 h-[2px] mx-0.5 rounded-full ${isComplete ? 'bg-blue-300' : 'bg-slate-200'}`} />
                                    )}
                                </div>
                            );
                        })}
                ...
                <div className="max-w-5xl mx-auto">
                        ...
                        <MotionDiv initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                            <div className="grid grid-cols-1 xl:grid-cols-[1.45fr_1fr] gap-4">
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="flex items-center gap-2 mb-1.5">
                                                        <div className={`p-1.5 rounded-md ${isSelected ? 'text-white' : 'bg-slate-100 text-slate-500'}`} style={isSelected ? {background: 'linear-gradient(135deg, #2563eb, #1d4ed8)'} : {}}>
                                ...
                                <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="flex items-center justify-between gap-3 mb-4">
                                        <div>
                                            <p className="text-[11px] uppercase tracking-[0.18em] text-blue-400 font-bold">Scrape Profile</p>
                                            <h3 className="text-xs font-bold text-slate-800 mt-1">Current Request</h3>
                                        </div>
                                        <div className="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-600">
                                            {selectedFormats.length} outputs
                                        </div>
                                    </div>
                                    <div className="space-y-2.5">
                                        <div className="rounded-xl border border-blue-100 bg-blue-50/50 px-3 py-2.5">
                                            <p className="text-[10px] uppercase tracking-wide text-blue-500 font-bold mb-1">Country</p>
                                            <p className="text-[13px] font-semibold text-slate-900">{COUNTRY_OPTIONS.find((item) => item.code === country)?.label}</p>
                                        </div>
                                        <div className="rounded-xl border border-blue-100 bg-blue-50/50 px-3 py-2.5">
                                            <p className="text-[10px] uppercase tracking-wide text-blue-500 font-bold mb-1">Language</p>
                                            <p className="text-[13px] font-semibold text-slate-900">{LANGUAGE_OPTIONS.find((item) => item.value === language)?.label}</p>
                                        </div>
                                        <div className="rounded-xl border border-blue-100 bg-blue-50/50 px-3 py-2.5">
                                            <p className="text-[10px] uppercase tracking-wide text-blue-500 font-bold mb-1">Active Outputs</p>
                                            <p className="text-[13px] font-semibold text-slate-900">{selectedFormats.join(', ')}</p>
                                        </div>
                                        {selectedFormats.includes('screenshot') && (
                                            <div className="rounded-xl border border-blue-300 bg-blue-50 px-3 py-2.5">
                                                <p className="text-[10px] uppercase tracking-wide text-blue-600 font-bold mb-1">Screenshot</p>
                                        ...
                                        {!selectedFormats.includes('screenshot') && (
                                            <div className="rounded-xl border border-dashed border-blue-200 bg-blue-50/20 px-3 py-3 text-center">
                                                <ImageIcon size={16} className="text-blue-300 mx-auto mb-1.5" />
                                                <p className="text-[11px] font-semibold text-slate-500">Screenshot controls stay hidden until needed</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.15fr] gap-4">
                                <div className="bg-white border border-blue-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Country</label>
                                            <select
                                                value={country}
                                                onChange={(e) => setCountry(e.target.value)}
                                                className="w-full px-3 py-2 border border-blue-200 rounded-xl text-[13px] text-slate-900 bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                            >
                                        ...
                                        <div>
                                            <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Language</label>
                                            <select
                                                value={language}
                                ...
                                <AnimatePresence mode="wait">
                                    {selectedFormats.includes('screenshot') ? (
                                        <MotionDiv
                                            ...
                                            exit={{ opacity: 0, y: 6 }}
                                        className="bg-white border border-blue-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow"
                                        >
                                            <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between rounded-xl border border-blue-100 bg-blue-50/40 px-3 py-2">
                                                    ...
                                                    <label className="inline-flex items-center cursor-pointer">
                                                        ...
                                                        />
                                                        <span className="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:bg-blue-600 relative transition-colors after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:w-5 after:h-5 after:bg-white after:rounded-full after:transition-transform peer-checked:after:translate-x-5" />
                                                    </label>
                                                </div>

                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    <div>
                                                        <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Viewport preset</label>
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`rounded-xl border px-2 py-2 text-[11px] font-bold transition-all ${
                                                                        screenshotOptions.viewport_preset === presetKey
                                                                            ...
                                                                            : 'border-slate-200 bg-white text-slate-600 hover:bg-blue-50/30'
                                                                    }`}
                                                                >
                                                    ...
                                                    <div>
                                                        <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Image quality</label>
                                                        <select
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <div>
                                                        <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Viewport width</label>
                                                        <input
                                                            type="number"
                                                            min="320"
                                                            max="7680"
                                                    ...
                                                    <div>
                                                        <label className="block text-[11px] font-bold uppercase tracking-wide text-blue-500 mb-1.5">Viewport height</label>
                                                        <input
                                                            type="number"
                                                            min="240"
                                                            max="4320"
                            ...
                            <AnimatePresence>
                                {selectedFormats.includes('extract') && (
                                    ...
                                    >
                                        <div className="rounded-2xl p-5 shadow-sm border border-blue-200" style={{background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)'}}>
                                            <div className="flex items-center gap-2 mb-2">
                                                <Zap size={16} className="text-blue-600" />
                                                <label className="block text-sm font-bold text-blue-900">AI Extraction Prompt</label>
                                            </div>
                                            <p className="text-xs text-blue-700/80 mb-3">Describe the fields you want extracted into a structured table. Clear prompts make CSV and Excel downloads much more useful.</p>
                                            <textarea
                                                value={extractPrompt}
                                                onChange={(e) => setExtractPrompt(e.target.value)}
                                                placeholder="Example: Extract product name, price, rating, availability, and product URL."
                                                rows={4}
                                                className="w-full px-3 py-2.5 border border-blue-300 rounded-xl text-[13px] text-slate-900 bg-white/95 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none resize-none transition-all placeholder:text-slate-400"
                                            />
                                        </div>
                                    </MotionDiv>
                                )}
                            ...
                            <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="flex flex-col sm:flex-row gap-2">
                                    <div className="relative flex-1">
                                        <Link size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-400" />
                                        <input
                                            type="url"
                                            value={url}
                                            onChange={(e) => setUrl(e.target.value)}
                                            placeholder="https://example.com/page"
                                            className="w-full pl-8 pr-3 py-2.5 border border-blue-200 rounded-xl text-[13px] text-slate-900 placeholder:text-slate-400 bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                        />
                                    </div>
                                    <button
                                        onClick={handlePreview}
                                        disabled={!url || selectedFormats.length === 0 || loading || (selectedFormats.includes('extract') && !extractPrompt)}
                                        className="flex items-center justify-center gap-1.5 px-5 py-2.5 disabled:bg-slate-100 disabled:text-slate-400 text-white rounded-xl font-bold text-sm transition-all shadow-md hover:-translate-y-0.5"
                                        style={(!url || selectedFormats.length === 0 || loading) ? {} : {background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', boxShadow: '0 4px 14px rgba(37,99,235,0.35)'}}
                                    >
                        ...
                        <MotionDiv initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-xs text-blue-500">Confirm the page output before launching the batch scrape.</p>
                                </div>
                                <button onClick={() => setStep(1)} className="flex items-center gap-1.5 px-3 py-1.5 border border-blue-200 rounded-xl text-xs font-bold text-blue-600 hover:bg-blue-50 transition-colors">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Country</p>
                                    <p className="mt-1 text-sm font-semibold text-slate-900">{COUNTRY_OPTIONS.find((item) => item.code === country)?.label}</p>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Language</p>
                                    <p className="mt-1 text-sm font-semibold text-slate-900">{LANGUAGE_OPTIONS.find((item) => item.value === language)?.label}</p>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Screenshot</p>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {previewData.map((item, idx) => (
                                    <div key={idx} className="bg-white border border-blue-100 rounded-2xl overflow-hidden shadow-sm">
                                        <div className="bg-blue-50/60 px-4 py-2.5 border-b border-blue-100 flex items-center justify-between">
                                            <span className="text-xs font-bold text-blue-700 truncate">{item.url}</span>
                                            <span className="text-[10px] uppercase tracking-wide font-bold text-blue-400">{selectedFormats.length} outputs</span>
                                        </div>
                                                    ...
                                                    {key === 'screenshot_url' ? (
                                                        <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                                                            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                                                                <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1">Image URL</p>
                                                                <a
                                                                    href={String(value)}
                                                                    target="_blank"
                                                                    rel="noreferrer"
                                                                    className="block text-[12px] font-medium text-sky-700 break-all hover:underline"
                                                                >
                                                                    {String(value)}
                                                                </a>
                                                            </div>
                            ...
                            <div className="bg-white border border-blue-200 rounded-2xl p-5 shadow-sm relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-40 h-40 rounded-bl-[120px] -z-10" style={{background: 'radial-gradient(circle, rgba(37,99,235,0.06) 0%, transparent 70%)'}}></div>
                                ...
                                <textarea
                                    ...
                                    rows={5}
                                    className="w-full px-3 py-2.5 border border-blue-200 rounded-xl text-[13px] text-slate-900 placeholder:text-slate-400 font-mono bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none resize-none transition-all"
                                />
                                <div className="flex justify-end mt-4">
                                    <button
                                        onClick={handleExecute}
                                        disabled={loading || !urls.trim()}
                                        className="flex items-center gap-1.5 px-5 py-2.5 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl font-bold text-sm transition-all hover:-translate-y-0.5"
                                        style={loading || !urls.trim() ? {} : {background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', boxShadow: '0 4px 14px rgba(37,99,235,0.35)'}}
                                    >
                    ...
                    {step === 3 && (
                        <MotionDiv initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-5">
                            <div className="bg-white rounded-2xl border border-blue-200 shadow-sm p-6" style={{background: 'linear-gradient(135deg, #f0f6ff 0%, #ffffff 50%)'}}>
                                <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
                                    <div className="flex items-start gap-4">
                                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0" style={{background: 'linear-gradient(135deg, #10b981, #059669)'}}>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-blue-500 text-sm max-w-2xl">Review the scraped dataset below or download it directly as CSV or Excel. This view is optimized for quick QA before you move on.</p>
                                        </div>
                                    </div>
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 xl:min-w-[520px]">
                                        <div className="flex flex-wrap gap-2">
                                            <button
                                                ...
                                                className="flex items-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-2.5 text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50 disabled:opacity-50"
                                            >
                                            ...
                                            {batchResults.some((row) => row.screenshot_url) && (
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-2.5 text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50"
                                                >
                                        ...
                                        {onComplete && (
                                            <button
                                                ...
                                                className="shrink-0 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:-translate-y-0.5"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Total URLs</p>
                                    <p className="mt-2 text-2xl font-black text-slate-900">{batchResults.length.toLocaleString()}</p>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Successful</p>
                                    <p className="mt-2 text-2xl font-black text-emerald-600">{successfulRows.toLocaleString()}</p>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Failed</p>
                                    <p className="mt-2 text-2xl font-black text-rose-600">{failedRows.toLocaleString()}</p>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
                                    <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500">Formats Used</p>
                                    <p className="mt-2 text-sm font-bold text-slate-900">{selectedFormats.join(', ')}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                                <div className="lg:col-span-2 bg-white border border-blue-100 rounded-2xl shadow-sm overflow-hidden">
                                    <div className="px-5 py-4 border-b border-blue-100 flex items-center justify-between gap-3" style={{background: 'linear-gradient(135deg, #f0f6ff, #ffffff)'}}>
                                        <div>
                                            <h4 className="text-sm font-bold text-slate-900">Tabular Data Preview</h4>
                                            <p className="text-xs text-blue-500">All extracted columns are shown here. Scroll horizontally to inspect the full schema.</p>
                                        </div>
                                        <span className="text-[11px] uppercase tracking-wide font-bold text-blue-400">{exportRows.length} rows</span>
                                    </div>
                                            ...
                                            <thead style={{background: 'linear-gradient(135deg, #eff6ff, #dbeafe)'}}>
                                                <tr>
                                                    {previewColumns.map((column) => (
                                                        <th key={column} className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-wide text-blue-600 border-b border-blue-100">
                                                            {formatLabel(column)}
                                                        </th>
                                                    ))}
                                                </tr>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            ...
                                            <div className="flex justify-between gap-4">
                                                <span className="text-blue-500">Screenshot</span>
                                            ...
                                            {selectedFormats.includes('screenshot') && (
                                                <div className="flex justify-between gap-4">
                                                    <span className="text-blue-500">Viewport</span>
                                                    <span className="font-semibold text-slate-900 text-right">
                                    ...
                                    <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm">
                                        ...
                                        {executionMetadata && (
                                            <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/50 p-3">
                                                <p className="text-[11px] uppercase tracking-wide font-bold text-blue-500 mb-1">Execution Metadata</p>
